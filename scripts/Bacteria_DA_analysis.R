...
samples=intersect(colnames(bacteria),metadata$Old_Sample_ID)
bacteria=bacteria[,samples]
bacteria=bacteria[!(rownames(bacteria) %in% c("Unclassified","Unclassified Bacteria")),]
rownames(metadata)=metadata$Old_Sample_ID
metadata=metadata[samples,]
#####Beta diversity analysis
transmission.dist<-vegan::vegdist(t(bacteria), method='bray')
bacteria_data=as.data.frame(t(bacteria))
bacteria_data$Old_Sample_ID=rownames(bacteria_data)
combine=merge(bacteria_data,metadata,by=c("Old_Sample_ID"))
...
transmission.div<-adonis2(transmission.dist~ Infection_Household + age_group, data=combine, permutations = 9999, method="bray",block=Old_Subject_ID)
...
Age=as.factor(Age)
Individuals=as.factor(transmission_metadata$Old_Subject_ID)
...
bacteria_negative=bacteria[,metadata_negative$Old_Sample_ID]
bacteria_data=as.data.frame(t(bacteria_negative))
bacteria_data$Old_Sample_ID=rownames(bacteria_data)
combine=merge(bacteria_data,metadata_negative,by=c("Old_Sample_ID"))
combine$Age=as.numeric(combine$Age)
combine$age_group<-"None"
combine[which(combine$Age>=18),]$age_group<-"Adults"
combine[which(combine$Age<18),]$age_group<-"Children"
...
transmission.div<-adonis2(transmission.dist~ group + age_group, data=combine, permutations = 9999, method="bray",block=Old_Subject_ID)
